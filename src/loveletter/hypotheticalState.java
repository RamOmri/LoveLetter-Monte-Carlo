 ...
 * **/
public class hypotheticalState implements Cloneable{

  
  public int player;//the player who observes this outcome, or -1 for the game engine
  ...
  public Card[] deck; //the deck of remaining cards
  public int[] top; //the index of the top of the deck
  public boolean[][] known; //whether player knows another players card
  public boolean[] handmaid;
  public int[] scores; //the current score of each player
  public java.util.Random random;
  public int[] nextPlayer; //the index of the next player to draw a card (using Object reference so value is shared).
  public simAgent[] agents;
   ...
   * */
  public hypotheticalState(java.util.Random random, simAgent[] agents, Card[] hand, Card[] deck){
    num = agents.length;
    if(num<2 || num>4)
   ...
   * **/
  public void newRound(Card[] h, Card[] d) throws IllegalActionException{
   ...
   * **/
  public hypotheticalState playerState(int player) throws IllegalActionException{
   /* if(this.player!=-1) throw new IllegalActionException("Operation not permitted in player's state.");
    if(player<0 || num<=player) throw new IllegalArgumentException("Player out of range.");
    try{
      hypotheticalState s = (hypotheticalState)this.clone();
      s.player = player;
      return s;
    ...
    try{ 
      ...
      s.player = player;
      return s;
    }catch(CloneNotSupportedException e){System.out.println("fucked up bad:::::::::::: ");
      e.printStackTrace();
      return null;
    }
  }

  public hypotheticalState cloneState(){
   // hypotheticalState s = (hypotheticalState)this.clone();
   hypotheticalState s = null;
       try{
      s = (hypotheticalState) this.clone();
      s.hand = this.hand.clone();
      s.deck = this.deck.clone();
      s.agents = this.agents.clone();
      s.discards = this.discards.clone();
      s.discardCount = this.discardCount.clone();
      s.top = this.top.clone();
      s.known = this.known.clone();
      s.nextPlayer = this.nextPlayer.clone();
      s.agents = this.agents.clone();
